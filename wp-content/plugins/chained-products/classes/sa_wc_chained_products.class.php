							...
							<tr class="item" rel="<?php echo $index; ?>">
								<td class="thumb">
									<a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>" class="tips" data-tip="<?php
																				echo '<strong>'.__('Product ID:', SA_WC_Chained_Products::$text_domain).'</strong> '. $_product->id;
										echo '<br/><strong>'.__('Variation ID:', SA_WC_Chained_Products::$text_domain).'</strong> '; if (isset($_product->variation_id) && $_product->variation_id) echo $_product->variation_id; else echo '-';
																				echo '<br/><strong>'.__('Product SKU:', SA_WC_Chained_Products::$text_domain).'</strong> '; if ($_product->sku) echo $_product->sku; else echo '-';
								...
								<td class="name">
									...
									<div class="row-actions">
										<span class="trash"><a class="remove_row" href="#"><?php _e('Delete item', SA_WC_Chained_Products::$text_domain); ?></a> | </span>
										<span class="view"><a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>"><?php _e('View product', SA_WC_Chained_Products::$text_domain); ?></a>
									</div>
									...
									<table class="meta" cellspacing="0">
										<tfoot>
											<tr>
												<td colspan="3"><button class="add_meta button"><?php _e('Add&nbsp;meta', SA_WC_Chained_Products::$text_domain); ?></button></td>
											</tr>
										</tfoot>
								...
								<td class="tax_class" width="1%">
									<select class="tax_class" name="item_tax_class[<?php echo $loop; ?>]">
										...
										$classes_options[''] = __('Standard', SA_WC_Chained_Products::$text_domain);
								...
								<td class="line_subtotal" width="1%">
									<label><?php _e('Cost', SA_WC_Chained_Products::$text_domain); ?>: <input type="text" name="line_subtotal[<?php echo $index; ?>]" placeholder="0.00" value="<?php echo esc_attr( number_format(0) ); ?>" class="line_subtotal" /></label>
									
									<label><?php _e('Tax', SA_WC_Chained_Products::$text_domain); ?>: <input type="text" name="line_subtotal_tax[<?php echo $index; ?>]" placeholder="0.00" class="line_subtotal_tax" /></label>
								</td>
								
								<td class="line_total" width="1%">
									<label><?php _e('Cost', SA_WC_Chained_Products::$text_domain); ?>: <input type="text" name="line_total[<?php echo $index; ?>]" placeholder="0.00" value="<?php echo esc_attr( number_format(0) ); ?>" class="line_total" /></label>
									
									<label><?php _e('Tax', SA_WC_Chained_Products::$text_domain); ?>: <input type="text" name="line_tax[<?php echo $index; ?>]" placeholder="0.00" class="line_tax" /></label>
								</td>
			...
			function chained_products_add_row(chained_id) {
				jQuery('#add_chained_products_row_'+chained_id).off('click').on( 'click', function() { 
					...
					var new_row = " <p class='form-field' id='chained_products_row_"+chained_id+"_"+row_id+"'>\
										...
										<?php if ( Chained_Products_WC_Compatibility::is_wc_gte_23() ){?>\
											<input type='hidden' class='wc-product-search ajax_chosen_select_products_and_only_variations_"+chained_id+"' style='width: 50%;' id='chained_products_ids_"+chained_id+"_"+row_id+"' name='chained_products_ids["+chained_id+"]["+row_id+"]' data-placeholder='<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>' \
											data-action='woocommerce_json_search_products_and_variations' data-exclude='<?php echo $total_ids_to_exclude; ?>' data-multiple='true'/>\
										<?php } else { ?>\
											<select id='chained_products_ids_"+chained_id+"_"+row_id+"' name='chained_products_ids["+chained_id+"]["+row_id+"]' class='ajax_chosen_select_products_and_only_variations_"+chained_id+"' multiple='multiple' data-placeholder='<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>'>\
											</select>\
										<?php } ?>\
