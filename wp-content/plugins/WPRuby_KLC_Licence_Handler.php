...
* 
*/
class WPRuby_KLC_Licence_Handler
{
		...
		if(version_compare(WC()->version, '2.6.0', 'lt')){
			...
			if(isset($_GET['page']) && $_GET['page'] == $this->page){
				if(isset($_GET['tab']) && $_GET['tab'] == 'shipping'){
					if(isset($_GET['instance_id']) && is_numeric($_GET['instance_id'])){
						$method_class = get_class( WC_Shipping_Zones::get_shipping_method( $_GET['instance_id'] ) );
						if($method_class == 'WC_Australian_Post_Shipping_Method'){
							if(get_option($this->licence_option_key)=='' || get_option($this->licence_option_key .'_license_status')!='valid'){
								wp_redirect($this->redirect_url . '&instance_id=' . $_GET['instance_id']);
								exit;
							}						
						}
					}
